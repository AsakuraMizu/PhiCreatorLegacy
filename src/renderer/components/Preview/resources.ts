...
export const loaded: Record<keyof typeof raw, Texture> = {
  Drag: Texture.EMPTY,
...
export async function preload(): Promise<void> {
  if (loaded.Effect !== Texture.EMPTY) return;
  const loader = Loader.shared;
  Object.entries(raw).forEach(([name, url]) => loader.add(name, url));
  return new Promise((resolve, reject) => {
    loader.onError.add((err: unknown) => reject(err));
    loader.load(() => {
      Object.keys(raw).forEach((name) => {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        loaded[<keyof typeof raw>name] = loader.resources[name].texture!;
      });
      resolve();
    });
  });
