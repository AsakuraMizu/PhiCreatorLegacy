...
import Player, { PreviewOptions } from './Player';
import { getBase64, load } from './utils';
...
export default class UiRenderer {
  ...
  async setup({ title, diff, background }: PreviewOptions): Promise<void> {
    ...
    if (background) {
      ...
      const url = await getBase64(background);
      loader.add(url);
      await load(loader);
  
      const back = new Sprite(loader.resources[url].texture);
